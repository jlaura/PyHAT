    ...
    while end_of_libs_table is False:
        ...
        if 'INSTRUMENT_TEMPERATURE' in line[0]:
            instrument_temps = line[1]\
                             ...
                             +str(f.readline(),'utf-8').replace('\r','').replace('\n','')\
                             +str(f.readline(),'utf-8').replace('\r','').replace('\n','')
            instrument_temps = [float(i) for i in instrument_temps.replace('<degC>','').replace('(','').replace(')','').replace(' ','').split(',')]
            ...
            instrument_temps_name = instrument_temps_name.split('=')[1]\
                            ...
                            +str(f.readline(),'utf-8').replace('\r','').replace('\n','')\
                            +str(f.readline(),'utf-8').replace('\r','').replace('\n','')
            instrument_temps_name = instrument_temps_name.replace(' ','').replace('(','').replace(')','').replace('"','').split(',')
            f.readline()
            pass
        try:
